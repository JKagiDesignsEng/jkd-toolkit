...
function Add-NetworkPasswordButton {
  ...
  $tt.SetToolTip(
    $btn,
    "Lists saved Wi-Fi networks and the stored passwords (if available). This requires administrator rights and will print results to the console. Do not run on shared machines without permission."
  )
  $btn.Tag = @{ ToolTip = $tt }
  ...
  try {
      ...
      if (Get-Command -Name Invoke-WithDivider -ErrorAction SilentlyContinue) {
        Invoke-WithDivider -Action $action
      } else {
        & $action
      }
    } catch {
      ...
      # Ensure this runs even if the handler returned early or threw an exception.
